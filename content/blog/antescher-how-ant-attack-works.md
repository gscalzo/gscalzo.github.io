...
Everything below was measured from the tape image.
...
The group at `0xBA00` gives itself away: 16 non-blank bytes out of 64 in all four slots, where every other group runs from 22 to 53. A tiny object with a large transparent surround: the grenade. The group at `0xBD00` is the only one whose four slots vary wildly (15, 44, 53, 33), so it isn't four rotations of one thing, it's a sequence. Explosion stages.
